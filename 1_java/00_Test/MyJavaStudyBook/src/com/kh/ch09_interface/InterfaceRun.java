...
public class InterfaceRun {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 	접근제환자 abstract class 클래스명 {}
		 * 
		 * ->자식클래스에서 강제로 오버라이딩하여 동일한 형식의 메소드를 정의하고자 할 때
		 * 	자식 클래스마다 구현 내용은 다르지만 동일한 형식을 적용(메소드 통일성 / 표준확된 틀 제공)
		 * 
		 * => 추상메소드가 없는 경우도 추상클래스로 만들 수 있음
		 * ->어떤 경우 ? 해당 클래스가 미완성된 클래스라고 부여하고자 할 때
		 * 			객체 생성 불가능하게 하고자 할 때 
		 * 
		 * *상속(구현) 관계
		 * -클래스에서 클래스를 상속받을 때 : 자식클래스 extends 부모클래스 (단일 상속만 가능)				   : 화살표 실선		
		 * -클래스에서 인터페이스를 구현할때 :클래스 implements 인터페이스, 인터페이스,...(다중 상속 가능) 		:화살표 점선
