...
public class Variable {
 ...
 public static void printVariable() {
	  ...
	  * - 재사용성 증사 (한번 값을 저장하여 필요할 때마다 변수이름으로 가져다가 사용)
	  * - 유지 보수가 쉬어짐 (한번 저장해놓은 위치에 값을 변경함으로써)
	  */
 ...
 public static void declareVariable() {
	 ...
	 /*
	  * 변수의 선언 :값을 기록하기 위한 변수를 매모리 공간에 할당/확보
	  * 
	  * [표현법]
	  * 	 자료형 변수형;
	  * 
	  * -자료형 : 변수의 크기를 및 모양을 지정하는 부분
	  * - 변수형 : 변수의 이름을 부여하는 부분(의미 부여!)
	  * 
	  * 명명규칙 
	  * 	[1] 카멜케이스 : 소문자로 시작하고, 다른 단어가 붙을땐 대문자로 시작
	  * 	=> 클래스명의 겨우 : 대문자로 시작하도록 지어줌.
	  * 	[2] 대소문자를 구분함
	  * 	[3] 숫자로 시작하면 안됨, 예약어 사용 불가
	  * 	[4] 특수문자는 '$' 또는 '_' 만 사용 가능
	  * 
	  * 주의사항 
	  *  	-같은 영역 안 ({//}) 에서는 동일한 변수면 선언 불가(중복 불가)
	  ...
	  */ 
	 	...
	 	isFalse = 2*3 < 0;   //isFalse의 값 : isFalse
	 	
	 	//2. 숫자형(정수/ 실수)
	 	//2-1. 정수형 (소수점 x. 딱 떨어지는 수)
	 	// 		byte(1B)/ short(2B)/int(4B) iong(8B)
	 	byte bNum; 	// 1byte (
	 	bNum = -128; 
	 	
	 //bNum = 1000;	//범위를 벗어나면 오류발생! (byte형) = (int형)
	 	
	//2-2. 실수형 (소수점 o)
	//		float (4B) / double (8B)
	 	float fNum = 0.0f; // 0.0 값을 저장
	 						// 소수점 7자리까지 표현 가능
	 						// 값 자체(리터럴)를 저장할 때 값 뒤에 f를 붙어줌
	 	...
	 	//[표현법] final 자료형 상수명;
	 	final int AGE;	//명명규칙 대문자로만 사, 단어사이는 '_' 구분
	 	AGE = 10;
	 	//AGE = 20;		// 값이 한 번 저장된 이후에, 변경된 불가능!
	 	...
	 	//참고 
	 	int temp = 999_999_999; //가족성을 위해 3자리마다 _ 표시 가능!
