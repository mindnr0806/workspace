...
public class JqAjax2Controller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//* 문자열 하나의 데이터로 응답
		/*
		String result = "여름: " + name + ", 나이: " + (age == -1 ? "알수앖음" : age);
		...
		*/
		 ...
		 * (JavaScript Object Notation: 자바스크립트의 객체 표기법)
		...
		jsonArr.add(name);							//["아이유"]
		jsonArr.add(age);							//["아이유", 30]		
		
		//response.setContentType("text/html; charset=UTF-8");
		// => 응답 데이터가 문자열(String)타입으로 전달됨
		
		response.setContentType("application/json; charset=UTF-8");
		// => 문서 형식(mime type)을 JSON 형태(application/json)로 설정해야 함
		response.getWriter().print(jsonArr);
		*/
		
		//-- 일반 객체(JSONObject)에 담아 응답 --> HashMap과 유사
		JSONObject jsonObj = new JSONObject(); 		//{}
		jsonObj.put("name", name);					//{name: "아이네"}
		jsonObj.put("age", age);					//{name: "아이네"}
		
		response.setContentType("applixation/json; charset= UTF-8");
