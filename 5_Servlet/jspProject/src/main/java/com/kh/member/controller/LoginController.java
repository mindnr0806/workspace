...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// POST 요청 시 인코딩 처리 필요 (요청 시 전달되는 데이터에 한글이 포함된 경우 필요!)
		 ...
		 * 					 jsp, servlet에서 접근 가능
		 * [3] request     : 요청된 request 객체가 가지고 있음. 포워딩된 jsp페이지에서만 접근 가능
		 * [4] page        : jsp페이지에서 생성하고 해당 페이지 내에서만 사용 가능.
		 */
		
		// 처리된 결과에 따라 사용자가 보게 될 응답화면을 지정하여 포워딩 또는 url 재요청
		if (member == null) {
			// 조회 결과가 없는 경우  =>  로그인 실패 ( 에러페이지? 에러메시지? )
			...
			// 로그인된 사용자 정보를 session 영역에 저장
			HttpSession session = request.getSession();
			session.setAttribute("loginUser", member);	// menubar.jsp 페이지에서 로그인 유무를 확인할 때 사용
