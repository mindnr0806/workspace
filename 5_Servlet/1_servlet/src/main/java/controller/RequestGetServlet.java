...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Servlet -> DAO > DB 작업
		/*
		 * int = new MemberService().insert(userName, height, address, ...);
		 * if (result > 0 ) {
		 * // 성공
		 * }else{
		 *  //실패
		...
		pw.printf("<span>%s</span>살 입니다. ",age);
		
		if(foods == null) {
			pw.println("좋아하는 음식은 없읍니다.");
		}else {
			pw.printf("좋아하는 음식 :<span> %s",String.join("/", foods));
		}
