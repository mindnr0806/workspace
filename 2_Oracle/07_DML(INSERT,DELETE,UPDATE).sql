...
INSERT INTO EMPLOYEE
    VALUES (300, '양민욱', '930806-100000', 'MINWOOK@OR,KR', '01012345678',
    'D4', 'J4', 5000000, 0.3, NULL, SYSDATE, NULL, 'N');

/*
    2)INSERT INTO 테이블명(컬럼명,컬럴명,컬럼명) VALUES (값, 값, 값)
    ->컬럼을 직접 제시하여 해당 컬럼에 대한 값민을 추가
     제시되지 않은 컬럼에 대한 값은 기본적으로 NULL 값이 저장되고
     기본값에 대한 옵션(DEFAULT)이 있는 경우 해당 기본값으로 저장됨
     ->NOT NULL 재약조건이 있는 컬럼의 직접 컬럼을 제시하여 값을 추가하거나
        DEFAULT 옵션을 설정해야 함(그렇지 않으면 오류발생)
*/
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, EMAIL, JOB_CODE)
    VALUES(301, '양준혁', '000501-300000', 'junhyeok2@or.kr', 'J7');
---------------------------------------------------------------------------------
/*
...
/*
    ...
    UPDATE 테이블명 
        SET 컬럼 = 값,
            컬럼 = 값, ....
 [WHERE 조건식] --> WHERE절을 생략했을 경우 테이블의 모든 행의 SET절에 제시한 컬럼의 데이터가 변경   
    
    *업데이트 시 제약조건을 잘 확인해여함
*/
--DEPT_TABLE 테이블에 DEPARTMENT테이블 복제
...
--부서코드가 'D9'인 부서의 부서명을 '전력기획팀' 으로 변경
UPDATE DEPT_TABLE
     DEPT_TITLE = '전략기획팀'
    WHERE DEPT_ID = 'D9';
...
--전체 사원의 급여를 기존 급여애 10프로 인상 (기존급여 * 1.1)
UPDATE EMP_TABLE
...
DELETE FROM EMPLOYEE
WHERE EMP_ID = 'D1';
--> D1 값을 EMPLOYEE 테이블 사용 중이기 때문에 삭제 불가 (외래기 설정되어 있음)
