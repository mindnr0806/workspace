...
--전체 사원의 평균급여보다 더 많은 급여를 밝는 사원의 정보를 조회
...
/*
    *서브쿼리의 종류*
    서브쿼리를 수행한 결과값이 몇행 몇열로 나오나에따라 분류
    -단일행 서브쿼리 : 서브쿼리의 수행 결과가 오로지 1개일 때 (1행 1열)
    -다중행 서브쿼리 : 서브쿼리의 수행 결과가 여러행일 때(N행 1열)
    -다중열 서브쿼리 : 서브쿼리의 수행 결과가 한 행이고 여러개의 컬럼일 때 (1행 N일)
    -다중행 다중열 서브쿼리 : 서브쿼리의 수행 결과가 여러핼 여러 컬럼일 때 (N행 N열)
    >>종류에 따라 서브쿼리 앞에 붙는 연산자가 달라짐


--단일행 따라 : 서브쿼리 결과가 오로지 1개일 때
*/
...
--2)부서별 그\급여합이 17700000인 부서의 부서코드, 급여합 조회
...
/*
    ...
    >ANY (서브쿼리) : 여러 개의 결과값 중에서 하나라도 큰 경우가 잇다면 조회
    <ANY (서브쿼리) : 여러 개의 결과값 중에서 하나라도 작을 경우가 있다명 조회
        * 비교대상 > 값1 OR 비교대상 > 값2 OR 비교대상 > 결과값3...
    
    >ALL(서브쿼리) : 여러 개의 모든 결과값보다 클 경우 조회
    <ALL(서브쿼리) : 여러 개의 모든 결과값보타 작을 경우 조회
...
*/
--하이ㅏ유 사원과 같은 부서코드, 같은 직급코드에 해당하는 사원 정보를 조회
...
--박나라 사원과 같은 직급코드, 같은 사수를 가지고있는 사원 정보 조회(사원명, 직급코드, 사수번호)
...
-------------------------------------------------------------------------------
/*
    다중행 다중열 서브쿼리 : 서브쿼리의 결과가 여러행, 여라개의 열인 경우 
*/
...
/*
    인라인 뷰 : 서브쿼리를 FROM절에 사용하는 것
            => 서브쿼리의 수행 경과를 마치 데이블처럼 사용하는 것
*/
--사원들의 사번, 이름, 보너스 포함 연봉, 부시코드를 조회
--*보너스 포함 연봉이 NULL이 아니어야 하고, 보너스 포함 연봉이3000만원 이상인 사원들만 조회
-- => NVL : NULL인 경우 다른 값으로 변경
SELECT EMP_ID 사번, EMP_NAME 이름, (SALARY + (SALARY * NVL(BONUS, 0)))*12 "보너스 포함 연봉", DEPT_CODE
FROM EMPLOYEE
WHERE (SALARY + (SALARY * NVL(BONUS, 0)))*12 >= 30000000
ORDER BY 3 DESC;

SELECT ROWNUM, 사번 이름, "보너스 포함 연봉", 부서코드
FROM (SELECT EMP_ID 사번, EMP_NAME 이름, (SALARY + (SALARY * NVL(BONUS, 0)))*12 "보너스 포함 연봉", DEPT_CODE
                FROM EMPLOYEE
                WHERE (SALARY + (SALARY * NVL(BONUS, 0)))*12 >= 30000000
                ORDER BY 3 DESC;)
WHERE ROWNUM <= 5;               
                
--가장 최근에 입사한 ㅅ가원 5명을 조회
...
/*
        ...
        -DANSE() OVER(WJDFUFRLWNS) : 동일한 순위가 있더라도 그 다음 등수를 +1해서 순위 계산
...
SELECT EMP_NAME, SALARY,DESC() OVER(ORDER BY SALARY DESC)"순위"
FROM EMPLOYEE;
...
2)부서별 평균급여가 270만원을 초과하는 부서에 해당하는 부서코드, 부서별 총 급여함, 부사별 평균급여. 부서별 사원수를 조회
SELECT DENSE_CODE, SUM(SALARY)"총합", FLOOR(AVG(SALARY)) AS "평균", COUNT(*)"인원수"
FROM EMPLOYEE
...
-- 문제집 (원인)
--부서별 평균급여를 확인해야하는데, 사원 개개인의 급여를 확읺했다
