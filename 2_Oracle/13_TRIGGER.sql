/*
    *트리거 (TRIGGER)
    :지정한 테이블에 DML(INSERT, UPDATE, DELETE)문에 의해 변경사항이 있을 때 : 테이블에 이벤트가 발셍헸을때
        자동으로 매번 실행한 내용을 미리 정의해두는 것
        
        EX)회원 탈퇴시 기존 회원테이블에서 데이터 삭제(DELETE)하고
        탈퇴 회원 테이블에 데이터 추가(INSERT) 해야 할 때 자동으로 실핼
        
        EX)신고횟수가 특정 값을 넘어갔을 때 해당 회원을 블랙리스트 처리
        
        EX)입충고에 대한 데이터를 기록할 때 해당 상품의 재고 수량을 갱신해야 할 때
*/
/* 
    *트리거의 종류
    ...
    +문장 트리거 : 이벤트가 발생한 SQL문에 딱 한번만 트리거 실행
 ...
 *트리거 생성하기
    ...
    BEFORE|AFTER INSERT|UPDATE|DELETE ON 테이블명  --이벤트 관련정보
...
END;
 ...
 INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, DEPT_CODE, JOB_CODE, HIRE_DATE)
 VALUES(501, '김인창', '990574-3000000', 'D6', 'J5', SYSDATE);
 --------------------------------------------------------------------------
--상품 입고 및 출고 관련
...
--상품번호로 사용할 시퀀스 (시작번호: 200, 증가값: 5, 캐시메모리X)
...
DROP TRIGGER TRG_02;
 ...
 INSERT INTO TB_PDETAIL VALUES (SEQ_DNO.NEXTVAL, 200 SYSDATE, 100, '출고');
