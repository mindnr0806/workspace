...
-- JOB 테이블에서 직급이름만 조회
SELECT JOB_NAME 직급 FROM JOB;

-- DEPARTMENT 테이블의 모든 정보를 조회
SELECT * FROM DEPARTMENT;

-- 사원 테이블(EMPLOYEE)의 사원명(EMP_NAME), 이메일(EMAIL), 연락처(PHONE), 입사일(HIRE_DATE), 급여(SALARY) 조회
SELECT EMP_NAME 사원명, EMAIL 이메일, PHONE 연락처, HIRE_DATE 입사일, SALARY 급여
FROM EMPLOYEE;
...
SELECT EMP_NAME "사원명", SALARY AS "급여", HIRE_DATE 입사일, SALARY*12 AS 연봉
FROM EMPLOYEE
...
--=> 와일드카드로 사용되는 문자와 컬럼에 담긴 문자가 동일하기 때문에 모두 와일드카드로 인시됨!
...
-- 부서 배치를 받지 않았지만(=> 부서코드 정보가 없음. DEPT_CODE의 값이 NULL!),
-- 보너스를 받고 있는 사원(BONUS의 값이 NULL이 아님!)의 사원명, 보너스, 부서코드 조회
