...
/*
    ...
    *권한 회수 
    ...
    -->TEST 계정에 부여했던 KH계정의 EMPLOYEE 테이블을 조회;할 수 있는 권한을 회수
*/
/*
    ROLE(롤, 규칙) : 특정 권한들을 하나의 집합으로 모아놓은 것
    
    -CONNECT : 접속 권한 (CREATE SESSION)
    -RESOURCE : 지원 관리(특정 객체 생성 권한)(CREATE TABLE, CREATE VIEW,...)
...
/*
    ...
    *트랜잭션 종류 
        -COMMIT : 트랜잭션에 담겨져있는 변경사항들을 실제 DB애 적용하겠다를 의미
...
--사번이 217, 214 인 사원 삭제 -->DML, 트랜잭션 처리 필요함
DELETE FROM EMP01
...
COMMIT;     --변경사항 적용 (DB에 볌경사항을 적용)
ROLLBACK;   --마지막 COMMIT 시점으로 돌아감 (217, 214 사원이 삭제된 시점)
-----------------------------------------------------------------------------------

--208번 209, 210번 사원을 삭제
DELETE FROM EMP01
...
--SP 시점으로 롤백
ROLLBACK TO SP;
COMMIT;   --SP 시점으로 돌아간 뒤 COMMIT을 했기 때문 DELETE 208, 209, 210만 DP에 적용됨
--------------------------------------------------------------------------------
SELECT * FROM EMP01;
--221번 사원 삭제
...
-- DDL문 사용전 DML로 사용된 쿼리들이 있다면 확실하게 처리(COMMIT/ROLLBACK)한 후애 DDL을 사용해여 함
